...
export const sendCoopInvite = async (
  ...
  // Try Supabase first
  if (supabase && USE_SUPABASE_INVITES) {
    try {
...
export const getPendingInvitesForUser = async (
  ...
  return invites.filter(
    (inv) => inv.toUsername === username && inv.status === "PENDING",
  );
};
...
export const hasPendingInviteForAchievement = async (
  ...
  return (
    sentInvites.find(
      (inv) => inv.achievementId === achievementId && inv.status === "PENDING",
    ) || null
  );
};
 ...
 */
export const acceptCoopInvite = async (
  inviteId: string,
): Promise<CoopInvite | null> => {
  // Try Supabase first
  if (supabase && USE_SUPABASE_INVITES) {
